...
PATH=$JAVA_HOME/bin:/opt/arm-linux-androideabi-4.7/bin:$PATH
export PATH
...
function cleanv()
{
	 repo forall -c git clean -xdf; repo forall -c git reset --hard $1;
}
...
function zipsymbol()
{
    ...
    if [ -e "$ANDROID_PRODUCT_OUT/symbols/BPLGUInfoCustomApp_MT6575_*" ]; then
        rm "$ANDROID_PRODUCT_OUT/symbols/BPLGUInfoCustomApp_MT6575_*"
    fi
...
function lgesign()
{
    ...
    -e ActivityNetwork.apk=vendor/lge/security/platform -e LocationEM.apk=vendor/lge/security/platform \
    -e MediaTekLocationProvider.apk=vendor/lge/security/platform -e MobileLog.apk=vendor/lge/security/platform -e ModemLog.apk=vendor/lge/security/platform \
