...
#endif
struct tpd_driver_t *g_tpd_drv; //kaka_12_0110
...
//#ifdef TPD_TYPE_CAPACITIVE
	...
	if(touch_type == 1){
//	input_mt_init_slots(tpd->dev, TPD_MAX_POINTS, 0);
	/*
	if((tpd->dev->mt=kcalloc(TPD_MAX_POINTS, sizeof(struct input_mt_slot), GFP_KERNEL))==NULL) return -ENOMEM;
	tpd->dev->mtsize = TPD_MAX_POINTS;
	for (i-0; i< TPD_MAX_POINTS; i++)
    	{
    	    input_mt_set_value(&tpd->dev->mt[i], ABS_MT_TRACKING_ID, -1);
    	}		*/
...
//    	set_bit(ABS_MT_SLOT, tpd->dev->absbit);
...
//		input_set_abs_params(tpd->dev, ABS_MT_SLOT, 0, TPD_MAX_POINTS - 1, 0, 0); 	
