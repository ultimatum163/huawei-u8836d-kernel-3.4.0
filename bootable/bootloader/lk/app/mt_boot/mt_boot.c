...
int boot_linux_from_storage(void)
{
	...
	switch(g_boot_mode)
	{
		...
		case ALARM_BOOT:
			...
			strlen += sprintf(commandline, "%s%s%x%s%x", commandline, NAND_MANF_CMDLINE, nand_flash_man_code, NAND_DEV_CMDLINE, nand_flash_dev_id);
#endif
	...
	boot_linux((void *)CFG_BOOTIMG_LOAD_ADDR, (unsigned *)CFG_BOOTARGS_ADDR, (char *)commanline, board_machtype(), (void *)CFG_RAMDISK_LOAD_ADDR, g_rimg_sz);
}
...
void boot_linux_from_sd(void *kernel, unsigned *tags, char *cmdline, unsigned machtype, void *ramdisk, unsigned ramdisk_size)
{
	mtk_wdt_disable();
	while(1){};
	mtk_wdt_init();
...
void mt_boot_init(const struct app_descriptor *app)
{
	...
	if (g_boot_mode == MICROSD_BOOT)
	{
		g_boot_mode = NORMAL_BOOT;
		boot_linux_from_sd(NULL, NULL, NULL, 0, NULL, 0);
	}
